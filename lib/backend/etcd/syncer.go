...
func (syn *etcdSyncer) sendUpdate(key string, value *string, revision uint64) {
	...
	if value != nil {
		parsedValue = model.ParseValue(parsedKey, []byte(value))
	}
...
func (syn *etcdSyncer) sendDeletions(deletedKeys []string, revision uint64) {
	updates := make([]model.KVPair, len(deletedKeys))
	for ii, key := range deletedKeys {
		parsedKey := model.ParseKey(key)
		if parsedKey == nil {
			glog.V(3).Infof("Skipping unknown etcd key %v", key)
			continue
		}
		updates[ii] = model.KVPair{
			Key:      parsedKey,
			Value:    nil,
			Revision: revision,
		}
